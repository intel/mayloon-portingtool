...
org.eclipse.core.runtime.adaptor.EclipseClassLoader.$EclipseClassLoader$EclipseClasspathEntry$ ();
}
...
c$.$EclipseClassLoader$EclipseClasspathEntry$ = function () {
Clazz.pu$h ();
c$ = Clazz.decorateAsClass (function () {
...
this.mf = null;
return this.mf;
}var mfEntry = this.getBundleFile ().getEntry ("META-INF/MANIFEST.MF");
if (mfEntry != null) try {
var manIn = mfEntry.getInputStream ();
this.mf =  new java.util.jar.Manifest (manIn);
manIn.close ();
} catch (e) {
...
var fragCPs = this.b$["org.eclipse.core.runtime.adaptor.EclipseClassLoader"].getFragClasspaths ();
if (fragCPs != null) for (var i = 0; i < fragCPs.length; i++) if (this.getBundleFile () === fragCPs[i].getBundleData ().getBaseBundleFile ()) return (fragCPs[i].getBundleData ()).$hasPackageInfo;

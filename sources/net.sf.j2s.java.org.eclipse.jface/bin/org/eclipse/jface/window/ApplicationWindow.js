...
org.eclipse.jface.window.ApplicationWindow.$ApplicationWindow$ApplicationWindowLayout$ ();
}
...
$wt.custom.BusyIndicator.showWhile (display, ((Clazz.isClassDefined ("org.eclipse.jface.window.ApplicationWindow$1") ? 0 : org.eclipse.jface.window.ApplicationWindow.$ApplicationWindow$1$ ()), Clazz.innerTypeInstance (org.eclipse.jface.window.ApplicationWindow$1, this, Clazz.cloneFinals ("runnable", runnable, "fork", fork, "mgr", mgr, "display", display, "holder", holder))));
if (holder[0] != null) {
...
c$.$ApplicationWindow$ApplicationWindowLayout$ = function () {
Clazz.pu$h ();
c$ = Clazz.decorateAsClass (function () {
...
var result =  new $wt.graphics.Point (0, 0);
var ws = composite.getChildren ();
for (var i = 0; i < ws.length; i++) {
var w = ws[i];
var hide = false;
if (this.b$["org.eclipse.jface.window.ApplicationWindow"].getToolBarControl () === w) {
if (!this.b$["org.eclipse.jface.window.ApplicationWindow"].toolBarChildrenExist ()) {
hide = true;
result.y += 23;
}} else if (this.b$["org.eclipse.jface.window.ApplicationWindow"].getCoolBarControl () === w) {
if (!this.b$["org.eclipse.jface.window.ApplicationWindow"].coolBarChildrenExist ()) {
hide = true;
result.y += 23;
}} else if (this.b$["org.eclipse.jface.window.ApplicationWindow"].statusLineManager != null && this.b$["org.eclipse.jface.window.ApplicationWindow"].statusLineManager.getControl () === w) {
} else if (i > 0) {
hide = false;
}if (!hide) {
var e = w.computeSize (wHint, hHint, flushCache);
...
var w = ws[i];
if (i == 0) {
var e = w.computeSize (-1, -1, flushCache);
w.setBounds (clientArea.x, clientArea.y, clientArea.width, e.y);
clientArea.y += e.y;
clientArea.height -= e.y;
} else if (this.b$["org.eclipse.jface.window.ApplicationWindow"].getToolBarControl () === w) {
if (this.b$["org.eclipse.jface.window.ApplicationWindow"].toolBarChildrenExist ()) {
var e = w.computeSize (-1, -1, flushCache);
w.setBounds (clientArea.x, clientArea.y, clientArea.width, e.y);
clientArea.y += e.y + 2;
...
if (this.b$["org.eclipse.jface.window.ApplicationWindow"].coolBarChildrenExist ()) {
var e = w.computeSize (clientArea.width, -1, flushCache);
w.setBounds (clientArea.x, clientArea.y, clientArea.width, e.y);
clientArea.y += e.y + 2;
clientArea.height -= e.y + 2;
}} else if (this.b$["org.eclipse.jface.window.ApplicationWindow"].statusLineManager != null && this.b$["org.eclipse.jface.window.ApplicationWindow"].statusLineManager.getControl () === w) {
...
c$.$ApplicationWindow$1$ = function () {
Clazz.pu$h ();

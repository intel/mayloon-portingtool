...
org.eclipse.jface.action.StatusLine.$StatusLine$StatusLineLayout$ ();
}
...
this.addDisposeListener (((Clazz.isClassDefined ("org.eclipse.jface.action.StatusLine$1") ? 0 : org.eclipse.jface.action.StatusLine.$StatusLine$1$ ()), Clazz.innerTypeInstance (org.eclipse.jface.action.StatusLine$1, this, null)));
...
this.fCancelButton.addSelectionListener (((Clazz.isClassDefined ("org.eclipse.jface.action.StatusLine$2") ? 0 : org.eclipse.jface.action.StatusLine.$StatusLine$2$ ()), Clazz.innerTypeInstance (org.eclipse.jface.action.StatusLine$2, this, null)));
this.fCancelButton.addDisposeListener (((Clazz.isClassDefined ("org.eclipse.jface.action.StatusLine$3") ? 0 : org.eclipse.jface.action.StatusLine.$StatusLine$3$ ()), Clazz.innerTypeInstance (org.eclipse.jface.action.StatusLine$3, this, null)));
...
var timer = ((Clazz.isClassDefined ("org.eclipse.jface.action.StatusLine$4") ? 0 : org.eclipse.jface.action.StatusLine.$StatusLine$4$ ()), Clazz.innerTypeInstance (org.eclipse.jface.action.StatusLine$4, this, Clazz.cloneFinals ("timestamp", timestamp, "animated", animated)));
...
c$.$StatusLine$StatusLineLayout$ = function () {
Clazz.pu$h ();
c$ = Clazz.decorateAsClass (function () {
...
function (composite, wHint, hHint, changed) {
...
var useWidth = true;
var w = children[i];
if (w === this.b$["org.eclipse.jface.action.StatusLine"].fProgressBarComposite && !this.b$["org.eclipse.jface.action.StatusLine"].fProgressIsVisible) useWidth = false;
 else if (w === this.b$["org.eclipse.jface.action.StatusLine"].fToolBar && !this.b$["org.eclipse.jface.action.StatusLine"].fCancelButtonIsVisible) useWidth = false;
var data = w.getLayoutData ();
if (data == null) data = this.DEFAULT_DATA;
var e = w.computeSize (data.widthHint, data.heightHint, changed);
if (useWidth) {
totalWidth += e.x;
totalCnt++;
}maxHeight = Math.max (maxHeight, e.y);
}
if (totalCnt > 0) totalWidth += (totalCnt - 1) * 3;
if (totalWidth <= 0) totalWidth = maxHeight * 4;
return  new $wt.graphics.Point (totalWidth, maxHeight);
...
var ws =  Clazz.newArray (count, 0);
var h = rect.height;
var totalWidth = -3;
for (var i = 0; i < count; i++) {
var w = children[i];
if (w === this.b$["org.eclipse.jface.action.StatusLine"].fProgressBarComposite && !this.b$["org.eclipse.jface.action.StatusLine"].fProgressIsVisible) continue ;if (w === this.b$["org.eclipse.jface.action.StatusLine"].fToolBar && !this.b$["org.eclipse.jface.action.StatusLine"].fCancelButtonIsVisible) continue ;var data = w.getLayoutData ();
if (data == null) data = this.DEFAULT_DATA;
var width = w.computeSize (data.widthHint, h, flushCache).x;
ws[i] = width;
totalWidth += width + 3;
}
var diff = rect.width - totalWidth;
ws[0] += diff;
var msgMinWidth = Math.floor (rect.width / 3);
if (ws[0] < msgMinWidth) {
diff = ws[0] - msgMinWidth;
ws[0] = msgMinWidth;
} else {
diff = 0;
}for (var i = count - 1; i >= 0 && diff < 0; --i) {
var min = Math.min (ws[i], -diff);
ws[i] -= min;
diff += min + 3;
}
var x = rect.x;
...
for (var i = 0; i < count; i++) {
var w = children[i];
if (w === this.b$["org.eclipse.jface.action.StatusLine"].fProgressBarComposite && !this.b$["org.eclipse.jface.action.StatusLine"].fProgressIsVisible || w === this.b$["org.eclipse.jface.action.StatusLine"].fToolBar && !this.b$["org.eclipse.jface.action.StatusLine"].fCancelButtonIsVisible) {
w.setBounds (x + rect.width, y, ws[i], h);
continue ;}w.setBounds (x, y, ws[i], h);
if (ws[i] > 0) x += ws[i] + 3;
}
}, "$wt.widgets.Composite,~B");
c$ = Clazz.p0p ();
};
c$.$StatusLine$1$ = function () {
Clazz.pu$h ();
